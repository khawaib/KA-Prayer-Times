<?php
/**
 * @version		1.5.x.1 prayertimes $
 * @package		prayertimes
 * @copyright	Copyright � 2009 - 2010 Khawaib.co.uk - All rights reserved.
...
class prayertimesModelImport extends JModel {
...
function _buildQuery() {
	...
	function upload() {
		...
		$filepath = JPATH_SITE.DS.'administrator'.DS.'components'.DS.$option.DS.'uploads'.DS;
	  	if (!$filename) {
	  		$app->redirect("index.php?option=$option", "Please select a file to upload.");
	  		return;
	  	}
	//    if (!eregi("(\.(sql|gz))$",$filename)) {
		if (!eregi("(\.(csv))$",$filename)) {
	  		$app->redirect("index.php?option=$option", "File of this type is not allowed.");
	  		return;
	  	}
	...
	//  	   }
			if (!JFile::upload($file['tmp_name'], $pathandfile)) {
				$app->redirect("index.php?option=$option", "Upload failed, My be you have restrictions on upload file size.");
				return;
			}
			
			if (!$this->parseCSVFile($filepath, $filename)) {
				$app->redirect("index.php?option=$option", "Could not Import CSV File.");
				return;
			}
	  	}
	    return true;
	  		...
	  		$databaseCharset = ($database->hasUTF()) ? 'utf8' : '';
	  			...
	  			if ( $fCharset == $databaseCharset && $fDriver == $databaseDriver) {
	  				...
	  				if ( !file_exists( $csvfile ) ) {
	            		$app->redirect("index.php?option=$option", "File does not exit. Path: $csvfile");
	      		  		return;
	  				}
...
while ( $arrData = fgetcsv ( $resFileHandler, 1000, ",","\"" )){
	if ($count > 1) {
		$query = sprintf('INSERT INTO `#__prayertimes` SET 
		`date`				= "%s",
		`fajr_begins`		= "%s",
		`fajr_jamaat`		= "%s",
		`sunrise`			= "%s",
